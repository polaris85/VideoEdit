...
- (void)needsReload {
    _flowLayout.itemSize = CGSizeMake(3.0, self.frame.size.height);
    [self reloadData];
}
...
{
    firstbar = 30000;
    MusicMergeController *controller =  (MusicMergeController *)parentController;
    [controller buttonfalse];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    NSMutableArray *rowsArray = [NSMutableArray arrayWithCapacity:[_collectionView.indexPathsForVisibleItems count]];
    ...
    {
        lastbar =[[rowsArray objectAtIndex:i] intValue];
        if(firstbar>lastbar)
            firstbar = lastbar;
    }
    NSString *startstr = [NSString stringWithFormat:@"%d", firstbar];
    NSUserDefaults *userdefaults = [NSUserDefaults standardUserDefaults];
    [userdefaults setValue:startstr forKey:@"start"];
    [userdefaults synchronize];
    NSLog(@"scroll end=%d", firstbar);
    
    MusicMergeController *controller =  (MusicMergeController *)parentController;
    [controller buttontrue];
}
